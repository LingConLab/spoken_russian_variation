---
title: Конструкции с отрицательной экзистенциальной связкой
author: Кьяра Наккарато
date: 'Last update: `r lubridate::make_datetime(year = 2025, month = 8, day = 19)`'
output:
  html_document:
...
readxl::read_xlsx('../../data/data.xlsx') |> 
  filter(feature_id == 11) ->
  db
```

Нестандартное маркирование подлежащих в конструкциях с отрицательной экзистенциальной связкой является распространенной чертой речи билингвов.  Для нескольких вариантов билингвального русского языка отмечается тенденция использовать именительный падеж вместо стандартного  родительного падежа при отрицании. Например, *газ не было* (чувашско-русский билингв).
