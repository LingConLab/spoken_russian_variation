---
title: Диалектные причастия
author: Светлана Земичева
date: 'Last update: `r lubridate::make_datetime(year = 2025, month = 5, day = 20)`'
output:
  html_document:
...
readxl::read_xlsx('../../data/data.xlsx') |> 
  filter(feature_id == 13) ->
  db
```

Стандартные причастия прошедшего времени в русском языке образуются с помощью суффиксов -*н*-/-*т*- (*построенный*, *умерший*). В диалектах наряду с ними используются нестандартные формы с суффиксом -*ши* (*построивши*, *умерши*). Первый тип (*построивши*) образован от переходных глаголов; второй тип (*умерши*) образован от непереходных глаголов.
